...
def main():
    '''generates on-demand request dataset and met from context.json'''
    context = load_json('_context.json') #load your context file as a dict
    # load your basic configs for met & dataset
    ds = load_json(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'config/dataset.json'))
    met = load_json(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'config/met.json'))
    # build input ds & met from context variables
    ds = build_ds(context, ds)
    met = build_met(context, met)
